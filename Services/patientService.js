...
class PatientService {
  ...
  static async getDiagnosis({ patient_id }) {
    ...
    const diagnoses = await Diagnosis.find({ patient: patient_id }).populate(
      "updatedBy",
  ...
  static async cancelAppointment({ patient_id, appointment_id }) {
    if (
      !mongoose.isValidObjectId(patient_id) ||
      !mongoose.isValidObjectId(appointment_id)
    ) {
    ...
    if (!appointment)
      throw new Error("Appointment not found or not yours to cancel");
  ...
  static async createAppointment({
    ...
    if (
      !mongoose.isValidObjectId(patient_id) ||
      !mongoose.isValidObjectId(doctor_id)
    ) {
  ...
  static async getTestInterpretation({ patient_id, doctor_id, test_id }) {
    if (
      !mongoose.isValidObjectId(patient_id) ||
      !mongoose.isValidObjectId(doctor_id) ||
      !mongoose.isValidObjectId(test_id)
    ) {
    ...
    const interpretation = test.interpretations.find(
      (i) => i.doctor.toString() === doctor_id
    );
    if (!interpretation) {
      return {
        message: "No interpretation from this doctor found for this test",
      };
    }
  ...
  static async requestTestInterpretation({ patient_id, doctor_id, test_id }) {
    ...
    return {
      message: "Interpretation request submitted to doctor",
      test_id,
      doctor_id,
    };
  }
